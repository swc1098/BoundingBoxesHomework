...
// generates an axis aligned box out of the model to world
// I don't really get it...what? Shouldn't I have to pass in the Oriented Bounding Box and then
// take it's information in order to even do this? 
void BoundingBoxClass::GenerateAxisAlignedBoundingBox(matrix4 a_m4ModeltoWorld)
{
	// get the maximum and minimum of the Oriented Bounding Box.
	vmax = GetMaximumOBB();
	vmin = GetMinimumOBB();

	// size is equal to some arbitrary value
	size = vector3(0,0,0);

	// get the maximum and minimum of all vertices in model
	vector4 aMax = a_m4ModeltoWorld * vector4(vmax , 0);
	vector4 aMin = a_m4ModeltoWorld * vector4(vmin , 0);

	// DEBUGGING
	//std::cout<< std::endl << "max x: " << aMax.x << std::endl;
	//std::cout<< std::endl << "max y: " << aMax.y << std::endl;
	//std::cout<< std::endl << "max z: " << aMax.z << std::endl;

	// generate a proper size based on aMin and aMax
	size.x = glm::distance(vector3(aMin.x,0.0f,0.0f),vector3(aMax.x,0.0f,0.0f));
	size.y = glm::distance(vector3(0.0f,aMin.y,0.0f),vector3(0.0f,aMax.y,0.0f));
	size.z = glm::distance(vector3(0.0f,0.0f,aMin.z),vector3(0.0f,0.0f,aMax.z));
...
void BoundingBoxClass::AddBoxToRenderList(matrix4 a_m4ModelToWorld, vector3 a_vColor, bool a_bRenderCentroid)
{
	...
	pMeshMngr->AddCubeToQueue(a_m4ModelToWorld * glm::translate(m_v3Centroid) * glm::scale(size),a_vColor,MERENDER::WIRE);
}